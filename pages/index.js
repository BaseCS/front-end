...
// Array of filters for different conferences
const filters = [
  {
    id: "ai",
    name: "AI",
...
// Define a function that joins classes while filtering out falsy values
...
export default function Example() {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        Header: "Count",
        accessor: (row) => row.FACULTY.length,
      },
      {
  ...
  // Define the inner columns of the table
  const innerColumns = useMemo(
    () => [
      {
        Header: "Faculty Members",
        accessor: (faculty) => removeDigits(faculty.properties.name),
      },
      {
        Header: "(Raw) Publications",
        // accessor: (faculty) => faculty.properties.notes,
      },
      {
        Header: "(Adjusted) Publications",
        // accessor: (faculty) => faculty.properties.homepage,
      },
    ],
    []
  );
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://localhost:8000/api/v0/institutions"
        );
        if (!response.ok) {
        ...
        // Sort data based on the length of FACULTY array in descending order
        data = data.sort((a, b) => b.FACULTY.length - a.FACULTY.length);
  ...
  // Configure the table using react-table
  const {
    getTableProps,
    getTableBodyProps,
    headerGroups,
  ...
  // Define the style for a dot used for checkboxes
  const dotStyle = {
    height: "4px",
    width: "4px",
  ...
  // Handle checkbox changes and log the state
  ...
  // Log checked boxes when they change
  useEffect(() => {
      ...
      <div className="bg-white">
        <div>
          ...
          <Transition.Root show={mobileFiltersOpen} as={Fragment}>
            ...
            >
              {/* Overlay for background when dialog is open */}
              <Transition.Child
                as={Fragment}
                              ...
                              <div className="space-y-6">
                                {section.options.map((option, optionIdx) => (
                                  <div key={option.value} className="flex-col">
                                    ...
                                    {/* Child Filters */}
                                    {option.children && (
                                      <div className="ml-6 space-y-2 flex-col">
                                        {" "}
                                        {option.children.map(
                                          (childOption, childOptionIdx) => (
                                            <div
          ...
          <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="relative z-10 flex items-baseline justify-between pt-12 pb-6 border-b border-gray-200">
              ...
              <div className="flex items-center">
                <a className="p-2 text-sm font-medium text-gray-600 hover:text-blue-600">
                  Methodology
                </a>
                <span className="text-gray-700">|</span>
                <a className="pl-2 py-2 text-sm font-medium text-gray-600 hover:text-blue-600">
                  For developers
                </a>
            ...
            {/* Introduction */}
            <h3 className="text-m text-blue-900 p-2">
              CSRankings is a metrics-based ranking of top computer science
              institutions around the world. Click on the plus icon to expand
              areas. Click on the dropdown icon to expand institutions.
            </h3>

            {/* Grid Layout */}
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-x-12 gap-y-10 pt-6">
              {/* Filters Section (visible on LG screens) */}
              <div className="hidden lg:block">
                {/* Title for Research Areas */}
                <h2
                  id="products-heading"
                  className="text-2xl font-bold text-gray-900 pb-2"
                >
                  Research Areas
                </h2>
                {/* Filters Form */}
                ...
                <div className="flex flex-col">
                  <div className="overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div className="inline-block min-w-full sm: lg:px-8">
                      ...
                      >
                        {/* Table Header */}
                        <thead className="bg-gray-50 text-center whitespace-nowrap">
                            ...
                            return (
                              <React.Fragment key={row.getRowProps().key}>
                                ...
                                <tr>
                                  <td>
                                    <span
                                      className="text-lg text-blue-500 p-2 rounded-full"
                                ...
                                <tr {...row.getRowProps()}>
                                  {row.cells.map((cell) => {
                                    ...
                                    return (
                                      <td
                                        {...cell.getCellProps()}
                                        className={`text-sm text-gray-900 font-light py-4 whitespace-nowrap ${
                                  ...
                                  <tr>
                                    <td>
                                      ...
                                      <table>
                                        <thead>
                                          <tr>
                                            {innerColumns.map((column) => (
                                              <th
                                                key={column.Header}
                                                className="text-sm text-center text-blue-700 pb-3 text-left"
                                              >
                                                {column.Header}
                                              </th>
                                            ))}
                                        ...
                                        <tbody>
                                          {/* Institution Subdata Category */}
                                          {row.original.FACULTY.map(
                                              ...
                                              <tr key={facultyIndex}>
                                                {innerColumns.map(
                                                  (column, columnIndex) => (
                                                    <td
                                                      key={columnIndex}
                                                      className={`text-sm text-center text-gray-900 font-light py-2 whitespace-nowrap ${
                                                        column.className ?? ""
                                                      }`}
                                                    >
...
// This function is used to fetch data from API endpoint on the server side.
export const getServerSideProps = async () => {
  // Send a GET request to the specified API endpoint (in this case, "http://localhost:3000/api/institutions")
  const res = await fetch("http://localhost:3000/api/institutions");
  
  // Parse the response data as JSON
  const data = await res.json();

  // Return an object with the fetched data as props
  // The data fetched from the API is passed to the component as "rankings" prop
